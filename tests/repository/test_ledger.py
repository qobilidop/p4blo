...
- the class counts in the summary table equal the entries' classes, and
  each entry's class equals the one pinned in tests/repository/data/ledger-classes.json,
  so two entries swapping classes is caught.

SpecTec names are checked against the pinned rule inventory by
tests/oracles/test_spectec_rules.py, which reads the same `SpecTec:` lines.
"""
...
def test_every_entry_keeps_its_pinned_class() -> None:
    ...
    assert not changed, (
        "entries whose class differs from tests/repository/data/ledger-classes.json:\n"
        + "\n".join(changed)
        + "\nA class change is a claim change: make it deliberately in"
        " docs/ir-semantics.md, then edit tests/repository/data/ledger-classes.json by hand to"
...
def test_the_test_resolver_is_strict() -> None:
    assert (
        reference_problem("tests/repository/test_ledger.py::test_the_test_resolver_is_strict")
        is None
    )
    assert reference_problem("tests/programs/corpus/stacks") is None
    assert reference_problem("tests/repository/test_ledger.py::test_no_such_test") is not None
    assert reference_problem("tests/programs/corpus/no_such_program") is not None
    # A helper is defined but is not a test.
    assert reference_problem("tests/repository/test_ledger.py::reference_problem") is not None
    assert reference_problem("tests/repository/test_ledger.py::Entry") is not None
