...
interpreter. It is built by `tests/oracles/build.sh`, driven by `tests/oracles/run.py`,
and asserted by `tests/oracles/test_oracle.py`, which skips without a binary and
...
SpecTec by name and `tests/oracles/test_spectec_rules.py` can check the citations
...
it once per pin with `python3 tests/oracles/coverage.py build` where
`build.sh` runs, then regenerate with `uv run python tests/oracles/coverage.py`
(`--check` compares, `--inputs DIR` adds generated pairs). Every in-scope
item that does not fire is explained by hand in
`spectec-coverage-exclusions.json`; `tests/oracles/test_spectec_coverage.py`
...
```
uv run python tests/oracles/run.py tests/programs/corpus/forwarder/forwarder.txtpb tests/programs/corpus/forwarder/*.stf
uv run pytest tests/oracles/test_oracle.py -v
```
...
`tests/oracles/test_oracle.py` pins the rendering.
...
```sh
uv run pytest tests/oracles/test_oracle_generated.py -v                   # seeds 0 to 59, as CI runs them
uv run python tests/oracles/generated.py --seeds 0:500 --out .artifacts/oracle/campaign
```
...
In `tests/oracles/test_oracle_generated.py`, seeds a defect explains are listed in
...
tests/oracles/p4blo.watsup -i p4c/p4include -i tests/oracles/include` process resident,
...
`tests/oracles/test_oracle_block.py` replays every corpus and example vector on the
...
```sh
uv run pytest tests/oracles/test_oracle_block.py -v    # about 45 seconds on an M-series Mac
```
...
`tests/oracles/test_frontend_spectec.py` skips without it, and fails instead under
...
`tests/oracles/frontend/catalog.py` pins; fetched once, sparsely, into
`~/.cache/p4blo/p4c-census`), translates each through `il-export`, replays
p4c's vector on the Python interpreter as `tests/oracles/frontend/p4c_stf.py` does,
...
`uv run python tests/oracles/frontend_census.py --check` reproduces it byte
...
spec elaboration included; the whole `tests/oracles/test_oracle.py` runs in
about four seconds.
...
unexplained `fail` or `error`. `tests/oracles/test_oracle_generated.py` takes
about three minutes.
...
setup path: `tests/oracles/build.sh`.
