...
submodule (the one tests/oracles/frontend/catalog.py pins), exports it with
P4-SpecTec's `il-export`, translates it with `p4blo.frontend`, and replays
p4c's own vector on the Python interpreter as
`tests/oracles/frontend/p4c_stf.replay` does. Each program gets one outcome:
...
numbers quoted elsewhere stay tied to a command.
    ...
    uv run python tests/oracles/frontend_census.py            # rewrite the result
    uv run python tests/oracles/frontend_census.py --check    # compare with it

The oracle is found as tests/oracles/run.py finds it (`$P4BLO_ORACLE_BIN`,
...
RESULT = Path(__file__).resolve().parents[2] / "tests/oracles/frontend-census.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if exporter is None or exporter.missing() is not None:
        ...
        print(f"frontend_census: {why}; run tests/oracles/build.sh", file=sys.stderr)
        return 2
