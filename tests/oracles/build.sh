...
# what is missing. See tests/oracles/README.md.
#
# The patches under tests/oracles/patches/ are applied to the checkout before
# the build, in name order; they add p4blo's block architecture with the
# `block` command (tests/oracles/block.py, which passes it
# tests/oracles/p4blo.watsup) and the `il-export` command
...
[ -e "${PATCHES[0]}" ] || { echo "[tests/oracles/build] no patches in $PATCHES_DIR" >&2; exit 1; }
...
log() { printf '[tests/oracles/build] %s\n' "$*" >&2; }
