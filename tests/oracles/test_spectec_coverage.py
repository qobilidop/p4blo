...
tests/oracles/spectec-coverage.json is the measurement tests/oracles/coverage.py
makes on the pinned simulator: which items of the rule inventory
(tests/oracles/spectec-rules.json) fire when the corpus and examples run.
tests/oracles/spectec-coverage-exclusions.json is written by hand: every
...
EXCLUSIONS = ROOT / "tests/oracles/spectec-coverage-exclusions.json"
INVENTORY = ROOT / "tests/oracles/spectec-rules.json"
SCRIPT = ROOT / "tests/oracles/coverage.py"
COVERAGE_DOC = ROOT / "docs" / "p4-spec-coverage.md"

sys.path.insert(0, str(ROOT))
from tests.oracles import coverage  # noqa: E402
...
# no leaf of their own (tests/oracles/coverage.py, on merged rules).
MERGED = {
...
# adding an exclusion (the Lean side's tests/conformance/coverage/unhit-tags.json is held
# the same way).
UNHIT_CEILING = 2
