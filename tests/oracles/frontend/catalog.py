...
Every file under tests/oracles/frontend/ but `probes/` is a verbatim copy, checked
...
- `tutorials/basic.p4`: p4lang/tutorials at the commit the tutorial
  ...
  forwarder's source. The firewall's own source is tests/oracles/firewall.p4.
...
(tests/oracles/test_frontend_spectec.py, section 5).
"""
...
HERE = Path(__file__).resolve().parents[3] / "tests/oracles/frontend"
...
CORPUS: tuple[CorpusSource, ...] = (
    ...
    CorpusSource("tutorial_firewall", ROOT / "tests/oracles/firewall.p4", "projected"),
