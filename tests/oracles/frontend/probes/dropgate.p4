// A packet mark_to_drop drops in ingress does not run egress.
...
control I(inout H hdr, inout M meta, inout standard_metadata_t sm) { apply { if (hdr.h.f == 0) { mark_to_drop(sm); } else { sm.egress_spec = 1; } } }
