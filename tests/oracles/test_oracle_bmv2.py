...
Docker image (tests/oracles/bmv2/Dockerfile), driven by tests/oracles/bmv2/run.py. Without
Docker or without the image every replay test skips and says how to build it;
with them, each `tests/programs/corpus/<program>/*.stf` must pass, where a divergence and an
oracle-side error (a construct the switch cannot load, a crash, a timeout)
are both failures, labeled apart as in tests/oracles/test_oracle.py: the second is not
a disagreement, but it leaves the claim unchecked.

One vector is a known, analysed divergence and is marked `xfail`: see
`KNOWN_DIVERGENCES` below and tests/oracles/bmv2/README.md. A second
...
# tests/oracles/frontend/catalog.py, and what BMv2 does with the corpus vector
...
def image() -> str:
    ...
    if reason is not None:
        pytest.skip(
            f"the BMv2 oracle is not available: {reason} (see tests/oracles/bmv2/README.md)"
        )
    return name
...
def test_use_last_rewrites_the_printed_last_index_form() -> None:
    ...
    # refuses to load, so the printed program is rewritten (tests/oracles/bmv2/run.py).
    p4 = (
