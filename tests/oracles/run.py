"""Replay corpus vectors on P4-SpecTec's simulator, the oracle for claim 2.
    ...
    uv run python tests/oracles/run.py <program.txtpb> <vectors.stf>
...
directory of tests/oracles/build.sh; the spec and the include directory are taken
...
class Oracle:
    ...
    def missing(self) -> str | None:
        ...
        if not (self.include / "v1model.p4").is_file():
            return f"{self.include} has no v1model.p4; tests/oracles/build.sh fetches it"
        return None
...
def find_oracle(environ: dict[str, str] | None = None) -> Oracle | None:
    ...
    tests/oracles/build.sh puts it.
    """
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="tests/oracles/run.py", description="replay STF vectors on P4-SpecTec's simulator"
    )
    ...
    if oracle is None:
        print(
            "no p4spectec binary: run tests/oracles/build.sh, "
