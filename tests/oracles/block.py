...
# the root itself for `tests.oracles.run`.
...
INCLUDE_DIR = Path(__file__).resolve().parents[2] / "tests/oracles/include"
...
WATSUP = Path(__file__).resolve().parents[2] / "tests/oracles/p4blo.watsup"
# The patches build.sh applies, and the stamp it writes after a build:
# "<commit> <digest of the patches>".
PATCHES_DIR = Path(__file__).resolve().parents[2] / "tests/oracles/patches"
...
class BlockOracle:
    ...
    def missing(self) -> str | None:
        ...
        if built[1:] != [patches_digest()]:
            return (
                f"{stamp} does not name the current tests/oracles/patches: "
                "rerun tests/oracles/build.sh"
            )
        return None
...
def run_block(
    ...
    if oracle is None:
        ...
        if oracle is None:
            raise RuntimeError("no p4spectec binary: run tests/oracles/build.sh")
