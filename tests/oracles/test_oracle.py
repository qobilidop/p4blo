...
The oracle is P4-SpecTec's `p4spectec sim`, built by tests/oracles/build.sh and
driven by tests/oracles/run.py. Without a built binary every replay test skips and
says so; with one, each `tests/programs/corpus/<program>/*.stf` must pass, where a
...
def test_translate_renders_lpm_prefixes_as_wildcards_with_priority(forwarder: ir.Index) -> None:
    ...
    # rule (tests/oracles/README.md): the wildcard form at the key's full width is
