...
tests/oracles/generated.py turns a seed into a program from one of the DRT's
...
simulator the oracle tests skip, as in tests/oracles/test_oracle.py; the
...
tests/oracles/README.md, "Generated programs".
"""
...
from tests.oracles import generated  # noqa: E402
...
def seed_param(seed: int) -> object:
    ...
    return pytest.param(
        ...
        marks=pytest.mark.xfail(
            ...
            reason=f"pinned SpecTec {KNOWN[seed]}; tests/oracles/generated.py, known_defect",
        ),
    )
...
def test_table_mask_model_uses_the_base_as_mask_and_ranks_ties() -> None:
    program = arch_wire.load_text(ROOT / "tests/programs/corpus/priority/priority.txtpb")
    (block, table) = next(
...
def lpm_precedence() -> tuple[apb.BlockAssembly, Case]:
    ...
    program = arch_wire.load_text(ROOT / "tests/programs/corpus/forwarder/forwarder.txtpb")
    vector = stf.parse((ROOT / "tests/programs/corpus/forwarder/lpm_precedence.stf").read_text())
...
# Corpus-family seeds (tests/programs/corpus/acl) whose outputs `exact_where_masked`
