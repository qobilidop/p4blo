"""Replay corpus vectors on BMv2's simple_switch, the second oracle for claim 2.
    ...
    uv run python tests/oracles/bmv2/run.py <program.txtpb> <vectors.stf>
...
(tests/oracles/bmv2/Dockerfile). Each vector is then
...
become pcap records, and the image's driver (tests/oracles/bmv2/driver.py) plays
...
def unavailable(image: str | None = None) -> str | None:
    ...
    if probe.returncode != 0:
        return f"the {image} image is not built: docker build -t {image} tests/oracles/bmv2"
    try:
        probe = subprocess.run(
...
def judge(plan: Plan, reply: dict) -> tuple[str, str]:
    ...
    expectation on the same port is not caught (tests/oracles/bmv2/README.md).
    """
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="tests/oracles/bmv2/run.py", description="replay STF vectors on BMv2's simple_switch"
    )
