...
and asserted by
[`tests/oracles/test_oracle_bmv2.py`](../test_oracle_bmv2.py), which
...
- **Longest-prefix matching.** The specification's simulator has no
  longest-prefix rule: `tests/oracles/run.py` has to hand it a priority equal
  to the prefix length, so the *translation* supplies the rule the
  semantics claims (`tests/oracles/README.md`, "The STF dialect"). BMv2's lpm
...
```
docker build -t p4blo-bmv2 tests/oracles/bmv2                         # a few minutes, then cached
uv run python tests/oracles/bmv2/run.py -v tests/programs/corpus/forwarder/forwarder.txtpb tests/programs/corpus/forwarder/*.stf
uv run pytest tests/oracles/test_oracle_bmv2.py -v                     # about 30 seconds for the corpus
```
...
pass, one is the divergence below. The whole `tests/oracles/test_oracle_bmv2.py`
...
oracle does correct is `tests/programs/corpus/register_bounds/README.md`, which says
...
divergence is carried here and in `tests/oracles/test_oracle_bmv2.py` as a
...
`tests/oracles/frontend/p4c/`) instead of the printed golden and replays
...
`tests/programs/corpus/priority/README.md`.
