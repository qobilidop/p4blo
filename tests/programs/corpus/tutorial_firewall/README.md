...
The unchanged original is pinned separately in `tests/oracles/firewall.p4`
by `tests/oracles/firewall.py`; the repository's root LICENSE contains
...
orders. `tests/programs/corpus/tutorial_firewall/test_firewall.py` asserts independently expected packet
...
production extern. `tests/programs/corpus/tutorial_firewall/test_firewall_boundaries.py` adds every byte
truncation of one frame and valid-malformed-valid persistence sequences;
`tests/programs/corpus/tutorial_firewall/test_firewall_generated.py` adds generated flow and host-policy
...
[BMv2 adapter](../../../oracles/bmv2/README.md#register-readback). The
...
`impl/python/tests/arch/test_crc.py` and the complete register observations exist.
...
`tests/programs/corpus/tutorial_firewall/test_firewall_semantics.py`, `test_firewall_body_semantics.py`
