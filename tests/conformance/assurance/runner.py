...
NODES = (
    "tests/programs/corpus/forwarder/test_forwarder_apply_semantics.py::test_lean_agrees_skip_default_packet_replay",
    "tests/programs/corpus/forwarder/test_forwarder_tables_semantics.py::test_lean_agrees_shortest_prefix_fault_replay",
    "tests/programs/corpus/tutorial_firewall/test_firewall_bloom_semantics.py::test_python_bloom_read_alias_is_not_expected",
...
def provenance(root: Path) -> dict[str, object]:
    ...
    paths.update(
        {Path("scripts/check-assurance.py"), Path("tests/conformance/assurance/test_runner.py")}
    )
    paths.update(p.relative_to(root) for p in (root / "tests/conformance/assurance").glob("*.py"))
    return {
...
def inputs() -> tuple[Input, ...]:
    ...
    require(
        firewall
        == arch_wire.load_text(
            ROOT / "tests/programs/corpus/tutorial_firewall/tutorial_firewall.txtpb"
        ),
