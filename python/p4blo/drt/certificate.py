...
def _observe(program: pb.Program, register_value: int, counter_value: int) -> Claim:
    ...
    return {
        "completion": completion,
        "register": [register.width, [hex(cell.value) for cell in register.cells]],
        "counter": [hex(value) for value in counter.counts],
        "local": [local.width, hex(local.value)] if isinstance(local, Bits) else None,
    }
