...
A `builtin`, `extern` or `tbl` prefix (a function the OCaml runtime
supplies, a relation or type the architecture supplies, or a function
defined by a table of rows) is stripped; the item is still part of the
architecture-free semantics as far as its callers go.
...
ITEM = re.compile(
    r"^\s*(?:(?:builtin|extern|tbl)\s+)?(?P<kind>syntax|relation|rule|rulegroup|dec)\s+"
