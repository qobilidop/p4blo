...
rule inventory tests/oracle/spectec-rules.json; tests link to their
files. `--check` compares instead of writing and exits non-zero on
drift, which is how tests/structure/test_ledger_xref.py keeps the table
true.
"""
...
# Sections of the ledger that hold no entries, as tests/structure/test_ledger.py
# has them.
...
def entries(text: str) -> list[Entry]:
    """The ledger's entries in order, parsed as tests/structure/test_ledger.py does."""
    found: list[Entry] = []
    section = ""
