...
| `DeviationLaws.header_equal_invalid`, `header_equal_valid_invalid`, `header_equal_valid`, `evaluate_eq_invalid_headers` | Header `==` compares validity first and fields only when both are valid, on `Value.equal` and through the evaluator from any run | That the compared headers share a type; anything about SpecTec, whose rule differs here |
| `DeviationLaws.evaluate_index_out_of_range`, `writeLValue_index_out_of_range`, `writeLValue_member_out_of_range` | An index at or past the size reads the zero invalid header of a declared element type; a write through it, or through one of its fields, leaves the run the operand reads left | Index and element typing; a field write whose path reads change the run |
| `DeviationLaws.evaluate_lastIndex`, `lastIndex_empty`, `evaluate_last_empty`, `evaluate_last_nonempty` | `lastIndex` is `nextIndex - 1` in `bit<32>`, so `2^32 - 1` at zero; `hs[hs.lastIndex]` reads the zero invalid header on an empty stack of fewer than `2^32` elements and element `nextIndex - 1` otherwise | The parser-only rule; that the eDSL or a frontend elaborates `hs.last` this way |
| `DeviationLaws.pushFront_spec`, `pushFront_clamp`, `popFront_spec`, `popFront_clamp` | Exact elements, size and `nextIndex` of the resulting stack value, including a count above the size | The statement's write back through its lvalue; the pop laws assume `nextIndex` at most the size |
| `DeviationLaws.evaluate_shl_large`, `evaluate_shr_large` | A shift by the width or more evaluates to zero through the evaluator, whatever the amount's width or size | Shifts below the width |
| `DeviationLaws.toBytes_padded`, `write_fits` | The emitter's bytes are the emitted bits followed by fewer than eight zero bits, for every emitter built by fitting writes | Deparser traversal; what an architecture appends |
| `DeviationLaws.enterState_revisit`, `enterState_again`, `step_state_revisit` | Entering a state at the cursor recorded for it raises `ParserTimeout` with the run unchanged, an immediate re-entry does so, and the machine step starts unwinding with it | That a loop without consumption reaches such an entry; parser termination |
| `DeviationLaws.lookup_hit`, `lookup_miss`, `lookup_longest_prefix` | A hit runs an installed entry matching every key with the longest total prefix, or the largest priority with a ternary key; a miss means none matches and runs the current default | Installation's canonicity and tie rejection; that the installed entries came from `Installed.build` |
