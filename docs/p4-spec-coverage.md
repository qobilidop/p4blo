...
| `tableEntryPriorityIR` (`priority = n`) | in | `Entry.priority`, larger wins | Decision: entry priority. Const entries' smaller-wins `@priority` and list order are renumbered. | translated from the typed IL's priorities, which already number entries without one |
