...
## Integration discovery follow-up

The reviewer independently collected the integrator's workflow selections:
BMv2 selects all six original-source full-state profiles; SpecTec selects
six passing profiles and both exact original/printed mask discrepancies.
The jobs build their oracle first and preserve the existing CRC/corpus steps.
All three corpus suites discover 11 programs and 17 vectors, and the six
original BMv2 profiles cover 30 prefixes. Scope/proof limitations were
inspected; the status row was clarified to say six profiles total, not six
additional profiles. No blocking integration finding remained.
