...
- **eDSL hoisting lacks the zeroing.** The ruling that a local declared
  without an initializer re-zeroes at every entry is implemented in the
  bridge; the eDSL still hoists a state-local to a block local without a
  zeroing assignment (`impl/python/p4blo/edsl/`). A small fix and a test.
