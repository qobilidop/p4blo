...
| Division of labor with `p4-spectec-lean` (the user's project, started 2026-09-25; read at its `fdaa520`: design and scaffolding, no code yet): A6 superseded, the oracle machinery frozen at maintenance, the bridge frozen at the corpus, `tests/oracle/p4blo.watsup` published as the block contract, the five consumed interfaces listed in the plan with their formats; two alignments recorded in the plan (its SpecTec pin `8c8e0c6f` differs from ours, to be matched at its first milestone; its program export replaces patch `0002` when it exists) | recorded in the plan and the register |
...
| D, test directory regrouping: `tests/{unit,codec,programs,drt,lean,external,structure}/` each with a README, no flat test file, the layout pinned; `docs/ledger-xref.md` generated by `scripts/ledger-xref.py` and checked for drift, every SpecTec citation linked to its line at the pin | merged, `3a654b5` |
| Gate speed: oracle suites deselected locally, tests in parallel with fixture-heavy modules pinned to one worker, the campaign test split into recorded parts; the whole gate script runs in about a minute and a half | done |
| Compaction of `.agents/` at the end of this scope | next |
