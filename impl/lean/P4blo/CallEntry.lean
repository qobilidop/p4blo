...
theorem source_entry_with_body (body : List P4bloIR.Stmt) (run : P4bloIR.Run) (hdr : Data headers) (metaData : Data metadata)
    (routeData : Data route) (observer : P4bloIR.Value)
    (continuation : List P4bloIR.Execution.Work)
    (hi : run.index = (WithBody.index body))
    ...
    ∃ zero, P4bloIR.Frame.forBlock (WithBody.index body) (WithBody.block body) = .ok zero ∧
      (boundFrame zero hdr.toValue metaData.toValue routeData.toValue observer).scope = WithBody.scope body ∧
      ...
      (P4bloIR.Execution.dispatch (.block "RewriteBody" args)).run run =
        (.ok [.runBlock (WithBody.block body), .blockReturn run.frame (WithBody.block body).params args],
      ...
      P4bloIR.Execution.Steps { work := .block "RewriteBody" args :: continuation, run }
        { work := .runBlock (WithBody.block body) :: .blockReturn run.frame (WithBody.block body).params args :: continuation,
          run := { run with frame := boundFrame zero hdr.toValue metaData.toValue routeData.toValue observer } } := by
  obtain ⟨zero, hz, hs, hb, hsource, hv⟩ := WithBody.initialized body
  ...
  · have hunrelated : (WithBody.scope body).vars["unrelated"]? = some (.var ⟨"unrelated", .bits 8⟩) := by cbv
    ...
    rfl
  · exact dispatch_entry run "RewriteBody" (WithBody.block body) zero hdr.toValue metaData.toValue routeData.toValue observer
      (by simpa [hi] using WithBody.block_lookup body) rfl (by simpa [hi] using hz) hh hm hr ho
  · exact entry_steps run "RewriteBody" (WithBody.block body) zero hdr.toValue metaData.toValue routeData.toValue observer continuation
      (by simpa [hi] using WithBody.block_lookup body) rfl (by simpa [hi] using hz) hh hm hr ho


/-- Original empty-body specialization, with its previous signature. -/
    ...
    ∃ zero, P4bloIR.Frame.forBlock CallEntry.index block = .ok zero ∧
      ...
      P4bloIR.Execution.Steps { work := .block "RewriteBody" args :: continuation, run }
        { work := .runBlock block :: .blockReturn run.frame block.params args :: continuation,
          run := { run with frame := boundFrame zero hdr.toValue metaData.toValue routeData.toValue observer } } :=
  source_entry_with_body [] run hdr metaData routeData observer continuation hi
    _callerBlockFrame hh hm hr ho
