...
def main : IO Unit := do
  ...
  P4blo.TutorialFirewallTests.run
  P4blo.ForwarderTableTests.run
  P4blo.TutorialFirewallBloomTests.run
  P4blo.ForwarderApplyTests.run
  let source ← IO.FS.readFile "../../spec/ir/P4bloIRTest/forwarder.json"
  let program ← IO.ofExcept (P4bloArch.BlockAssembly.fromJsonString source)
  ...
  match P4blo.prepareSwitch (default : P4bloArch.BlockAssembly) with
  | .error _ => pure ()
