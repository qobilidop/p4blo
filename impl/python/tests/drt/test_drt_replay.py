...
def test_both_clis_describe_state_only_divergences(
    ...
    cli.show(report, ROOT / "tests/programs/corpus/register_bounds", 1, None)
...
def test_ambiguous_peer_cannot_produce_false_agreement(tmp_path: Path, field: str) -> None:
    ...
    actual = python_outcome(v1model.load(program), cases[0], 4)
...
def test_replay_keeps_the_prefix_that_establishes_register_state(tmp_path: Path) -> None:
    ...
    def reset_each_packet(case: Case) -> Outcome:
        return python_outcome(v1model.load(program), case, 4)

    report = compare_cases("register_bounds", v1model.load(program), cases, 4, reset_each_packet)
    ...
    recovered = compare_cases(
        "register_bounds",
        v1model.load(recovered_program),
        recovered_cases,
        ports,
        reset_each_packet,
        seed,
...
def test_replay_preserves_values_stf_cannot_express(tmp_path: Path) -> None:
    ...
    report = compare_cases("register_bounds", v1model.load(program), cases, 4, lambda _: Outcome())
...
def test_matching_errors_are_not_a_successful_valid_input_campaign() -> None:
    ...
    report = compare_cases(
        "register_bounds",
        v1model.load(program),
        [case],
        4,
        lambda _: Outcome(
            error="ingress_port 99 is not a configured v1model port",
            state=snapshot(v1model.load(program)),
        ),
    )
...
def test_protocol_failure_preserves_the_experiment_and_prior_divergences(
    ...
    assert (
        cli.main(
            [str(ROOT / "tests/programs/corpus/register_bounds"), "2", "--save", str(tmp_path)]
        )
        == 2
    )
...
def test_report_inputs_do_not_alias_mutable_protobuf_entries() -> None:
    ...
    with pytest.raises(ProtocolError) as error:
        compare_cases("register_bounds", v1model.load(program), [case], 4, fail)
    report = error.value.report
...
def test_startup_failure_still_saves_concrete_inputs(tmp_path: Path) -> None:
    ...
    assert (
        cli.main(
            [
                str(ROOT / "tests/programs/corpus/register_bounds"),
                "2",
                "--lean",
                str(missing),
                "--save",
