...
open Lean P4bloIR P4bloArch

namespace AssemblyCodecTests

def exportValue (e : Export) : Json := Json.mkObj
  [("role", .str e.role), ("block", .str e.block)]
def programValue (p : BlockAssembly) : Json := Json.mkObj
...
private def mixed : BlockAssembly := ⟨⟨"program", ["last", "", "first"],
  [⟨"H", []⟩, ⟨"", []⟩, ⟨"H2", []⟩], [⟨"S", []⟩, ⟨"S2", []⟩, ⟨"", []⟩],
...
def tests : T Unit := do
  checkOk "program complete independent constructor" (BlockAssembly.fromJsonString mixedWire) (· == mixed)
  checkOk "program empty defaults" (BlockAssembly.decode "" empty) (· == emptyProgram)
  ...
  for (first, second) in order.zip order.tail do
    ...
    check "program adjacent first error"
      (match BlockAssembly.decode "" (Json.mkObj [(first, .bool false), (second, .bool false)]) with
       | .error e => e == first ++ ": expected " ++ what
       | .ok _ => false)
  check "program later nested bound"
    (BlockAssembly.fromJsonString "{\"header_types\":[{}, {\"fields\":[{\"type\":{\"bits\":4294967296}}]}]}" matches
      .error "header_types[1].fields[0].type.bits: 4294967296 does not fit in uint32")

end AssemblyCodecTests
